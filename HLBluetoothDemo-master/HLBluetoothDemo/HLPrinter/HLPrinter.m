...
- (void)setLittleOffsetText:(NSString *)text
{
    // 1.计算偏移量,因字体和字号不同，所以计算出来的宽度与实际宽度有误差(小字体与22字体计算值接近)
    NSDictionary *dict = @{NSFontAttributeName:[UIFont systemFontOfSize:22.0]};
    NSString *str = @"  ";
    NSString *tex = [str stringByAppendingString:text];
    NSAttributedString *valueAttr = [[NSAttributedString alloc] initWithString:tex attributes:dict];
    int valueWidth = valueAttr.size.width;
...
/////////////////
- (void)addLittleTitle:(NSString *)title value:(NSString *)value
{
    [self addLittleTitle:title value:value fontSize:HLFontSizeTitleSmalle];
}



- (void)addLittleTitle:(NSString *)title value:(NSString *)value fontSize:(HLFontSize)fontSize
{
    // 1.设置对齐方式
    [self setAlignment:HLTextAlignmentLeft];
    // 2.设置字号
    ...
    [self setText:title];
    // 4.设置实际值
    [self setLittleOffsetText:value];
    // 5.换行
    [self appendNewLine];
    if (fontSize != HLFontSizeTitleSmalle) {
        [self appendNewLine];

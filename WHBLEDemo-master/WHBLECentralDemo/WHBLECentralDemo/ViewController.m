...
@interface ViewController () <CBCentralManagerDelegate,CBPeripheralDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
//    [self.peripheral writeValue:data forCharacteristic:self.characteristic type:CBCharacteristicWriteWithResponse];
    NSInteger _limitLength = 256;
    // iOS 9 以后，系统添加了这个API来获取特性能写入的最大长度
    if ([self.peripheral respondsToSelector:@selector(maximumWriteValueLengthForType:)]) {
      _limitLength = [self.peripheral maximumWriteValueLengthForType:CBCharacteristicWriteWithResponse];
        NSLog(@"%ld",(long)_limitLength);
    }
    NSData * data = UIImageJPEGRepresentation([UIImage imageNamed:@"he.jpeg"], 0.1);
    NSInteger index = 0;
...
//        NSDictionary * dcit = @{@"flags":@"+",@"data":subData};
//        NSData * jsData = [NSJSONSerialization dataWithJSONObject:dcit options:msj error:nil];
        [self.peripheral writeValue:subData forCharacteristic:self.characteristic type:CBCharacteristicWriteWithResponse];
        NSLog(@"写入%ld",(long)index);
    }
    
    NSData *leftData = [data subdataWithRange:NSMakeRange(index, data.length - index)];
    if (leftData) {
//        NSDictionary * dcit = @{@"flags":@"none",@"data":leftData};
//        NSData * jsData = [NSJSONSerialization dataWithJSONObject:dcit options:NSJSONWritingPrettyPrinted error:nil];
